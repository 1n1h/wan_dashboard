...
import { Loader2, Play, Sparkles, Download, RotateCcw } from "lucide-react";
...
const STYLES = ["cinematic", "anime", "photoreal", "claymation", "noir", "studio ghibli"];
const ASPECTS = [
  { label: "16:9", code: "16:9" },
  { label: "9:16", code: "9:16" },
  { label: "1:1", code: "1:1" },
  { label: "21:9", code: "21:9" },
];
...
const T2V_MODELS = MODEL_REGISTRY["text-to-video"];

export function T2VStudio({ userTier = "free" as Tier }: { userTier?: Tier }) {
  const [modelId, setModelId] = useState(T2V_MODELS[0].id);
  ...
  const model = getModel("text-to-video", modelId) ?? T2V_MODELS[0];
  const cost = useMemo(
  ...
  async function onGenerate() {
    if (!prompt.trim() || status === "submitting" || status === "processing") return;
    ...
    try {
      const res = await fetch("/api/videos/generate", {
        ...
        body: JSON.stringify({
          ...
          ...(referenceImage.trim() && model.supportsReferenceImage
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[400px_1fr] h-full">
      ...
      <div className="border-r border-white/[0.06] overflow-y-auto p-6 space-y-6">
        <div>
          ...
          <h1 className="font-display text-2xl font-bold mb-1 text-white">Compose a clip</h1>
        ...
        <Field label="Prompt" hint={`${prompt.length}/500`}>
          <textarea
            value={prompt}
            onChange={(e) => setPrompt(e.target.value.slice(0, 500))}
            placeholder="A fox running through golden hour wheat fields, slow-motion, anamorphic lens"
            rows={5}
        ...
        <div className="flex-1 rounded-2xl bg-black/40 border border-white/[0.08] overflow-hidden grain aspect-[16/9] relative">
          ...
          <div className="absolute top-0 left-0 right-0 h-9 bg-black/50 backdrop-blur-md border-b border-white/[0.06] flex items-center px-4 gap-2 z-10">
            ...
            <div className="flex-1 text-center font-mono text-[10px] text-white/40 uppercase tracking-wider">
              wan://t2v · {resolution} · {aspect} · {duration}s
            </div>
            <div
              className={cn(
          ...
          <div className="absolute inset-0 pt-9 flex items-center justify-center">
            {/* Swap src to "/explainer-t2v.mp4" once a real explainer/example clip is in /public */}
            {status === "idle" && <PreviewExplainer src="/auth-bg.mp4" />}
            {status === "submitting" && <PreviewProcessing label="Submitting to GPU..." />}
            {status === "processing" && <PreviewProcessing label="Rendering frames..." />}
...
/**
 * Idle state: loops a short explainer/example video so the preview area
 * never feels empty. Falls back gracefully (just black bg) if the file
 * isn't present yet — drop your file at `public/explainer-t2v.mp4`.
 *
 * As soon as the user hits Generate, status flips off `idle` and this
 * component unmounts, so the explainer disappears and the loader/result
 * takes over the same canvas.
 */
    ...
    <div className="relative w-full h-full overflow-hidden">
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/55 backdrop-blur-md border border-white/10">
        ...
        <span className="font-mono text-[10px] uppercase tracking-wider text-white/80 whitespace-nowrap">
          Example output · enter a prompt to generate yours
        </span>
      </div>
    </div>
  );
